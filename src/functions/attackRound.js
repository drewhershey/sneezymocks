...
})
{
  ...
  if (cintaiValue > 0) 
    bonus += floor(cintaiValue / 20.0 * 3.0);    
  
  bonus -= floor(myLev * min(100, 4 * level) / 100);  
  if (isPc())    
    bonus += floor(myLev * max(10, offenseValue) / 100);
  else 
    bonus += floor(myLev * max(10, npcCombatValue) / 100);

  if (advOffenseValue > 0)
    bonus += floor(advOffenseValue / 4.0 * 3.0);
  ...
  if (isCasting)
    bonus -= floor(2 * myLev / 3);

  let positionBonus = 0;
  if (isSitting)
    positionBonus = floor(-1 * myLev / 4 + 1);
  else if (isRiding)
    positionBonus = floor(myLev / 4 + 1);
  else if (isFlying)
    positionBonus = floor(myLev / 3 + 1);
